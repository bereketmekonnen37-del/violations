...
const BossView = () => {
  ...
  const rawFiles = useAppSelector((s) => s.unfilteredContinuous.files);
  ...
  const cleanCount = useMemo(
    () => buildCleanContinuousRows(files, resolveProfile).length,
    [files, resolveProfile],
  );
  const masterCount = useMemo(
    () => buildMasterContinuousRows(allFiles, resolveProfile).length,
    [allFiles, resolveProfile],
  );
  const onDownload = () => {
    if (files.length === 0) return;
    downloadCleanContinuousCsv(files, resolveProfile);
  };
  const onDownloadMaster = () => {
    if (allFiles.length === 0) return;
    downloadMasterContinuousCsv(allFiles, resolveProfile);
  };
